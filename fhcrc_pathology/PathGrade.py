...
class PathGrade(SecondaryField):
    ...
    def __init__(self):        
        ...
        self.standardization_dictionary = {'high':'high','low':'low',
                                        'intermediate':'intermediate','3':'high',
                                        '1':'low','2':'intermediate','iii':'high',
                                        'i':'low','ii':'intermediate','moderate':'moderate',
                                        'poor':'poor','high':'high'}
        ...
        self.patterns = [(r'.*([\W]|^)('+self.strings3+r'.{,15}differentiat[ioned]+).*',2), (r'.*(grade.{,15}[\W]'+self.strings1+')([\W]|$).*',2),
                         (r'.*([\W]|^)('+self.strings1+r'[\W].{,15}grade).*',2),
                          (r'.*([\W]|^)(([123])[/of ]{1,6}3.{,20}fnclcc).*',2),(r'.*(fnclcc .{,20}([123])[/of ]{1,6}3)([\W]|$).*',1),
                         (r'.*([\W]|^)('+self.strings2+'.{,20}grade.{,5}[\W]'+self.strings1+')([\W]|$).*',2)]
