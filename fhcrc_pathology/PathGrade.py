...
def get_grade(text,line_onset):     
    ...
    for g,n in grades.items():
        m=re.match('.*('+g+'.{,15}grade).*',text,re.DOTALL)
        if m:
            grade_set.add(n)
            start_stops_set.add((m.start(1)+line_onset,m.end(1)+line_onset))
        m=re.match('.*(grade.{,15'+g+').*',text,re.DOTALL)
        if m:
            grade_set.add(n)
            start_stops_set.add((m.start(1)+line_onset,m.end(1)+line_onset))
    ...
    for each_desc in descriptions:
        m= re.match('.*('+each_desc+'.{1,15}differentiated).*',text,re.DOTALL)
        if m:           
            grade_set.add(m.group(1))                     
            start_stops_set.add((m.start(1)+line_onset,m.end(1)+line_onset))
       
    ## specific grading system (FNCLCC)
    m=re.match('.*(([123])[/of ]{1,6}3.{,20}fn[c]?l[c]?c).*',text,re.DOTALL)    
    if m:
        grade_set.add(grades[' '+m.group(1)+' '])
        start_stops_set.add((m.start(1)+line_onset,m.end(1)+line_onset))
    else: m=re.match('.*(fn[c]?l[c]?c .{,20}([123])[/of ]{1,6}3).*',text,re.DOTALL)
    if m:
        grade_set.add(grades[' '+m.group(1)+' '])
        start_stops_set.add((m.start(1)+line_onset,m.end(1)+line_onset))
    else: m=re.match('.*(fn[c]?l[c]?c .{,20}grade.{,5}([123])).*',text,re.DOTALL)
    if m:
        grade_set.add(grades[' '+m.group(1)+' '])
        start_stops_set.add((m.start(1)+line_onset,m.end(1)+line_onset))
