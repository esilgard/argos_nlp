...
def add_weights(text,stage_keyword,vector,matches3):
            '''
            add weights for engineered features to the instance vector
            '''
...
def get(note_list):
    ...
    for date_note in note_list:
        service_date=date_note[0][1]
        day_diff=(max_date-service_date).days+1        
        ...
        for stages,wordlist in word_d.items():            
            for matches1 in re.finditer('(('+'|'.join(wordlist)+').{,250}?('+'|'.join(lung_words)+'))',text):
                instance+=1
                vector1=''+date_note[0][0]+'_'+stages+'_'+str(instance)+' UNK'
                ...
                vector1+=' num_notes '+str(len(note_list))
            ...
            for matches2 in re.finditer('(('+'|'.join(lung_words)+').{,250}?('+'|'.join(wordlist)+'))',text):                           
                vector2=''+date_note[0][0]+'_'+stages+'_'+str(instance)+' UNK'
                ...
                vector2+=' num_notes '+str(len(note_list))
