    ...
    for lines in open(note_file,'r').readlines()[1:]:
        ...
        if len(line)>4:
            try:
                ...
                print 'ERROR PARSING DATETIME FROM NOTE FILE'
        ...
        for date_note in date_note_list:
            ...
            for stages,wordlist in word_d.items():
                ...
                for matches1 in re.finditer('(('+'|'.join(wordlist)+').{,250}?('+'|'.join(lung_words)+'))',text):
                    ...
                    tokens1=text[max(0,matches1.start()-70):min(len(text)-1,matches1.end()+70)].replace(stage_keyword1,' '+stage_keyword_no_space1+' ').split()
                ...
                for matches2 in re.finditer('(('+'|'.join(lung_words)+').{,250}?('+'|'.join(wordlist)+'))',text):                           
                    ...
                    tokens2=text[max(0,matches2.start()-70):min(len(text)-1,matches2.end()+70)].replace(stage_keyword2,' '+stage_keyword_no_space2+' ').split()
