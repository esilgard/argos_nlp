...
def reduce_confidence(confidence, num_notes):
    ...
    return confidence
## patterns and resources; these should be moved into individual text files ##
pre_neg_wordlist = ['mother',' mom ','father','dad','brother','sister','sibling',' uncle',' aunt','grandm','grandp',\
                    'grandf','versus', 'cousin',' no ',' not ','negative','free of','against','rule out',' r o ',' if ',\
                    'whether','unknown','consider','possibl','family','undetermined','would','potential','rather than',\
...
word_d = {'IV':['stage( i.{,4} )?[ ]*iv','stage[ ]*4[ab ]','metastatic','m1 ','distant metasta'],
        'III':['stage( i.{,4} )?[ ]*iii[ab]?','stage[ ]*3[ab ]','t4[ ]*n[01]?[ ]*.[^1]','t[1234][ ]*n3[ ]*.[^1]',
        't3[ ]*n[123][ ]*.[^1]','t[12][ab]?[ ]*n2[ ]*.[^1]','locally advanced','regional metasta','metastatic lymph'],
        'II':['stage( i.{,4} )?[ ]*ii[ab ]','stage[ ]*2[ab ]','t[12][ab]?[ ]*n1[ ]*.[^1]','t2[b]?[ ]*n0[ ]*.[^1]',
        't3[ ]*n0[ ]*.[^1]'],
        'I':['stage( i.{,4} )?[ ]*i[ab ]','stage[ ]*1[ab ]','t1[ab][ ]*(n0|n[^1])[ ]*[m]?[^1]','t2a[ ]*n0[ ]*[m]?[^1]']}
## because the current algorithm is written for lung specifically - these are all 'other' diseases ##
## TODO - make this list variable based on input arguments ##
other_wordlist = ['thyroid','breast','sarcoma ','uterine','brain','prostate','renal','colon','gastrointestin','rectum',\
                'pharyn','ulcer','decub','hiv','oral','head','esophag',' ovar','endometri','hepatic','liver','neuro','kidney',\
                'copd','ckd','ulcer','rectal',' bone','hepat','anal','bladder','hcc','gfr']
...
def get(note_list):
    ...
    if max(x[1] for x in system_answers) <= 0:
        ...
        ## basically an argmax of the candidate stage list ##
        ## TODO - make this 1-2 lines of code max ##
