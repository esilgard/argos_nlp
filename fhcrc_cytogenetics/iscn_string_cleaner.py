...
def get(original_text, karyo_offset):
    ...
    if not interpretation:
        ...
        if interpretation:            
            if 'nuc' not in text and 'FISH' not in text: swog = gb.INTERMEDIATE
            else:
                swog = gb.UNKNOWN
    else:
    ...
    if interpretation:
        ...
        return [{gb.NAME:gb.SWOG, gb.TABLE:gb.CYTOGENETICS, gb.VALUE:swog,gb.CONFIDENCE:.95,
                gb.STARTSTOPS:[{gb.START:text.find(interpretation_str), gb.STOP:text.find(interpretation_str) + \
                len(interpretation.group(1))}],gb.VERSION:__version__},
                {gb.NAME:gb.KARYOTYPE_STRING, gb.VALUE:text, gb.CONFIDENCE:.95,
                 gb.VERSION:__version__, gb.STARTSTOPS:[{gb.START:0}, {gb.STOP:len(text)}]}],text
    ...
    if 'ish' in karyotype_string or 'rror' in karyotype_string:        
        return [{gb.NAME:gb.SWOG, gb.TABLE:gb.AML_CYTOGENETICS, gb.VALUE:gb.UNKNOWN, gb.CONFIDENCE:0.0,
