    ...
    **to work correctly first line must contain the expected headers**
    ...
    try:       
        ...
        if set(header_set)>=(REQUIRED_HEADER_SET):           
            ...
            try:
                # sort records by MRN_CAPS, acc, and then setid - ignore null MRN_CAPSs, accs, or setids
                ...
                for line in obx:                    
                    ...
                    if gb.FILLER_ORDER_NO in line:                        
                        ...
                        cytogenetics_dictionary[mrn][acc][(-1,'FullText',0,None)] = cytogenetics_dictionary[mrn][acc].get\
                                                        ((-1,'FullText',0,None),'') + '\n'                        
                        chars_onset+=1                                                       
                    else:
                        ...
                        section_header=re.match('[\*\" ]*([A-Za-z ]+)[\*:]+',text)                                       
                        ...
                        cytogenetics_dictionary[mrn][acc][(section_order,section,chars_onset,specimen)]=cytogenetics_dictionary[mrn][acc].get((section_order,section,chars_onset,specimen),{})                
                        cytogenetics_dictionary[mrn][acc][(section_order,section,chars_onset,specimen)][index]=text
                        cytogenetics_dictionary[mrn][acc][(-1,'FullText',0,None)]=cytogenetics_dictionary[mrn][acc].get((-1,'FullText',0,None),'')+text+'\n'
                        
                        if 'RECEIVED' in text and 'CASE' in text:
                            received_date = re.match('.*RECEIVED:[ ]+([A-Z][a-z]+)[ ]+([\d]+)[ ]+([\d]{4}).*',text)   
                            if received_date:
                                cytogenetics_dictionary[mrn][acc][(-1,'Date',0,None)] = make_datetime.get((received_date.group(3),received_date.group(1),received_date.group(2)),'%Y,%b,%d')
                            else:  cytogenetics_dictionary[mrn][acc][(-1,'Date',0,None)]='NA'
