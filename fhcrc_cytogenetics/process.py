...
def get_fields(disease_group, disease_group_data_d,return_fields, karyotype_string, karyo_offset):
    ...
    {field names:[pertinent section(s)]} field_value_dictionary holds values for each of the fields in data_d
    '''
    ...
    for mrn in cytogenetics_d:
        for acc in cytogenetics_d[mrn]:
            ...
            if arguments.get('-a') == 'n':
                ...
                ## this means all cytogenetics algorithms will only be based on the single ISCN string
                for sections in sorted(cytogenetics_d[mrn][acc], key=lambda x: x[2]):               
                    ...
                    if 'ISCN' in section_header:                    
                        ...
                        for line, text in sorted(cytogenetics_d[mrn][acc][sections].items(),key=lambda x:x[0]):                        
                            cyto_string += text
                ...
                if cyto_string:                
                    ...
                    if not str_cleaner_return_dictionary:                    
                        ...
                        try:
                            ...
                            (disease_group, disease_group_data_d,return_fields, karyotype_string, karyo_offset)                            
                            field_value_output.append(field_value_dictionary)
                        except:
                            return (field_value_output, [{gb.ERR_TYPE:'Exception', gb.ERR_STR:\
                                     ' FATAL ERROR in process.get() - could not retrieve disease group specific \
                                    cytogenetics module' + str(sys.exc_info())}], list)           
                        if return_type != Exception:
                            field_value_dictionary[gb.TABLES] = []
                            field_value_dictionary[gb.TABLES].append({gb.TABLE:gb.CYTOGENETICS, gb.FIELDS:return_fields})                    
                            field_value_output.append(field_value_dictionary)
                            if return_errors:
